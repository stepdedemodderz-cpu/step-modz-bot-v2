...
export const RULES_DE = [
  ...
  '',
  '⚠️ Bitte bestätigt die Regeln mit einem Klick.'
].join('\n');
...
export const RULES_EN = [
  ...
  '',
  '⚠️ Age-restricted or obscene content is forbidden. This includes text, images and links containing nudity, sex, extreme violence or other graphic disturbing content.',
  '',
  ...
  '',
  '⚠️ Please confirm the rules with one click.'
].join('\n');

export function buildRulesIntroEmbed() {
  return new EmbedBuilder()
    .setTitle('📜 Regeln / Rules')
    .setDescription(
      [
        'Bitte wähle zuerst deine Sprache und lies die Regeln.',
        '',
        'Please choose your language first and read the rules.',
        '',
        'Danach bestätige die Regeln mit dem Button unten.',
        'After that, confirm the rules with the button below.'
      ].join('\n')
    )
    .setColor(0xf59e0b)
    .setFooter({ text: 'Step Mod!Z BOT • Regeln' })
    .setTimestamp();
}

export function buildRulesEmbed(language = 'de') {
  return new EmbedBuilder()
    .setTitle(language === 'en' ? '📜 Server Rules' : '📜 Server Regeln')
    .setDescription(language === 'en' ? RULES_EN : RULES_DE)
    .setColor(0xf59e0b)
    .setFooter({ text: 'Step Mod!Z BOT • Regeln' })
    .setTimestamp();
}
